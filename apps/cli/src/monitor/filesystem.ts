...
function getOpenFiles(pid: number): string[] {
  try {
...
function assessPath(filePath: string, cwd: string): { suspicious: boolean; critical: boolean; reason: string } {
...
export class FilesystemMonitor {
  ...
  scan(): FilesystemEvent[] {
    const newEvents: FilesystemEvent[] = []
    const pids = getAllPids()
    ...
    for (const pid of pids) {
      ...
      for (const filePath of files) {
        ...
        const event: FilesystemEvent = {
          ...
          severity: critical ? 'dangerous' : 'suspicious',
          message: reason,
        }
