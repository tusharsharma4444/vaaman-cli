...
import type { ScanResult, ThreatSignal, ThreatLevel, MonitorEvent } from './types.js'
...
function levelIcon(level: ThreatLevel): string {
  switch (level) {
...
export function printLiveSignal(signal: { level: ThreatLevel; reason: string }): void {
...
export function printHeader(packages: string): void {
  console.log()
...
export function printReport(result: ScanResult): void {
  ...
  if (result.signals.length > 0) {
    ...
    // Group by level
    const order: ThreatLevel[] = ['critical', 'dangerous', 'suspicious', 'clean']
