...
import { printHeader, printReport, printError, printWarning, } from './reporter.js';
...
program
    ...
    const block = opts.block !== false; // --no-block sets to false
    ...
    printHeader(npmArgs.length > 0
        ? npmArgs.join(' ')
        : '(from package.json)');
    // Check we're on Linux for full monitoring
    if (process.platform !== 'linux') {
        printWarning('Full monitoring requires Linux. ' +
